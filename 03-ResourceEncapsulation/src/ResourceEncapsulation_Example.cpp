...
/*  Berhane Tsegay
Cyber Security
Resource Encapsulation Examplar
*/
#include<iostream>
#include<string>
using namespace std;
...
void AccDetails()
{
    ...
    Customer custm;
    values money;
     //money = custm.transfermoney(); // method to transfer money, even Admin can't access this function because it is declared as private method to the customer class ( So that is encapsulation)
     custm.deposit(); // method to deposit money
    }
     else {
        cout << "Invalid username!! Sorry you are not ADMIN!" << endl;
		cout << "=========Please try with different login info ===========!" << endl;
		Customer cust;
	    cust.info();
     }
}
